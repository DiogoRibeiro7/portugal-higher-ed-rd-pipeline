...
def _write_inputs(tmp_path: Path) -> tuple[Path, Path, Path]:
    ...
    expected = pd.DataFrame(
        [
            {"unit_reference": "UID/00001/2023", "isced_f_scope": "05"},
            {"unit_reference": "UID/00002/2023", "isced_f_scope": "07"},
        ]
    )
    ...
    return participation_path, concordance_path, expected_path
...
def test_build_weights_uses_registered_count_and_equal_share_rules(tmp_path: Path) -> None:
    participation, concordance, expected = _write_inputs(tmp_path)
    output = build_weights(CONTRACT, participation, concordance, expected)
...
def test_build_weights_fails_when_participant_has_no_dgeec_mapping(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="Missing participant→DGEEC concordance"):
        build_weights(CONTRACT, participation, concordance, expected)
...
def test_primary_unit_registry_must_carry_frozen_field(tmp_path: Path) -> None:
    participation, concordance, expected = _write_inputs(tmp_path)
    frame = pd.read_csv(expected, dtype=str).drop(columns=["isced_f_scope"])
    frame.to_csv(expected, index=False)

    with pytest.raises(ValueError, match="isced_f_scope"):
        build_weights(CONTRACT, participation, concordance, expected)
