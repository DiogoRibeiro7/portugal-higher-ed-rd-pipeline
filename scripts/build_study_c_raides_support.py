...
def _row_value(values: list[Any], index: dict[str, int], name: str) -> Any:
    position = index[name]
...
def _xlsb_rows(path: Path) -> Iterable[dict[str, Any]]:
    with open_workbook(path) as book, book.get_sheet("Tabela1") as sheet:
        ...
        for row in sheet.rows():
            ...
            yield {
                "academic_year": _short_year(str(_row_value(values, index, "Ano letivo"))),
                "institution_code": _normalise_code(
                    _row_value(values, index, "Código do estabelecimento de ensino"), 4
                ),
                "institution_name": _row_value(values, index, "Estabelecimento de ensino"),
                "cycle": _row_value(values, index, "Curso/Ciclo de estudos"),
                "field": _normalise_code(
                    _row_value(
                    values,
                    index,
                    "Área de educação e formação - Código da área geral",
                ),
                2,
                ),
                "first_time": _row_value(values, index, "Primeira vez"),
                "count": _row_value(values, index, "N"),
            }
